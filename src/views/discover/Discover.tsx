...
function Discover() {
    ...
    const deletePostMutation = useMutation(( postId: Post['_id']) => deletePost(postId), {
        onSettled: () => {
    ...
    return <div className='flex flex-col flex-1'>
        <div className='toolbar bg-white flex items-center p-5 shadow-lg z-10'>
            ...
            <button className={`px-4 py-2 bg-lightgray text-primary !shadow-none ${classNames({ 'bg-primary text-white': showUsersPostsOnly })}`}
        ...
        <Masonry
            ...
            {displayedPosts.map(post => (
                <PostItem
                    ...
                    post={post}
                    style={{ maxHeight: 450 }}
                    {... (isBelongToCurrentUser(post) && { onDelete: () => deleteUserPost(post._id) })}
                />
            ))}
        </Masonry>
